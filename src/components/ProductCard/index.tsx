...
const ProductCard = ({
  ...
  const productAmount = useCartStore((state) =>
    state.getProductAmount(product.ProdCode),
  );
  const addProductInCart = useCartStore((state) => state.addProductInCart);
  const increaseProductAmount = useCartStore(
    (state) => state.increaseProductAmount,
  );
  const decreaseProductAmount = useCartStore(
    (state) => state.decreaseProductAmount,
  );
  ...
  const productImage = `https://10.10.220.1/api/GetProductPicture?ProdCode=${product.ProdCode}`;

  return (
    <div
      className={`${styles.productCardWrapper} ${className} ${withRightControl && styles.rightControll}`}
    >
      <div className={`${styles.imageWrapper}`} onClick={onImageWrapperClick}>
      ...
      <div className={styles.contentWrapper}>
        <h3>
          {i18n.language == "en" ? product.ProductNameENG : product.ProductName}
        </h3>
